...
class ItemList extends DBConnection
{
    ...
    public function displayItemsByCategory()
    {
        ...
        try {
            ...
            while ($itemRow = $getItemsPrepStmt->fetch()) {
                echo "<tr><td>" . htmlspecialchars($itemRow['categoryname'], ENT_QUOTES) . "</td><td>" . "<a href='change_item.php?itemid=" . htmlspecialchars($itemRow['itemid'], ENT_QUOTES) . "'>" . htmlspecialchars($itemRow['itemname'], ENT_QUOTES) . "</a>&#x25CF;" . htmlspecialchars($itemRow['unitname'], ENT_QUOTES) . "</td><td>" . htmlspecialchars($itemRow['notes']) . "</td></tr>\n";
            }
            echo "</table>\n";
    ...
    public function displayBuyTimeDetails($sortby)
    {
		...
		<th><a href='buy_time_details.php?sortby=last_buy_time_asc'>&#x25b2;</a>Last Buy Time<a href='buy_time_details.php?sortby=last_buy_time_desc'>&#x25bc</a></th>
            ...
            while ($itemRow = $getItemsPrepStmt->fetch()) {
                echo "<tr><td>" . "<a href='change_item.php?itemid=" . htmlspecialchars($itemRow['itemid'], ENT_QUOTES) . "'>" . htmlspecialchars($itemRow['itemname'], ENT_QUOTES) . "</a>&#x25CF;" . htmlspecialchars($itemRow['unitname'], ENT_QUOTES) . "</td><td>" . htmlspecialchars($itemRow['categoryname'], ENT_QUOTES) . "</td><td>" . htmlspecialchars($itemRow['buycount'], ENT_QUOTES) . "</td><td>" . htmlspecialchars($itemRow['lastbuytime'], ENT_QUOTES) . "</td></tr>\n";
            }
            echo "</table>\n";
        } catch (PDOException $exception) {
            echo "ERROR(" . __FILE__ . "): Could not display any itmes by category<br>\n";
